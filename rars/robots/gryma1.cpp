/* Car name: Gryma1
   ...
   This is my first attempt at a car for rars, it is mostly original, but doese
   borrow collision detection/avoidance from Jacbot

   This is GryMor's car #1 for the BORS mlwaukee race.
*/
...
static double sdiv(double a, double b)
   {
   if(b>0 && b<0.001)
      b=0.001;
   else if(b<0 && b>-0.001)
...
static double CritDist(double v0, double v1, double a)
   ...
   if(dv > 0.0)
      return(0.0);
   return sdiv((v0 + .5 * dv) * dv , a);
   }

con_vec Gryma1(situation &s)
   {
   ...
   if(s.damage > 15000)
      spd = (spd<80.0)?spd:80;
   alpha = sdiv(abaseadj*(s.to_lft-pos),width) - sdiv(acoradj * s.vn,s.v);
   vc = spd+spdadj;
   ...
   for(int i=0;i<5;i++) if (s.nearby[i].who<16)
      ...
      c_time = sdiv(-dot , vsqr);
      if(c_time > 3.0)
         continue;
      x_close = x + c_time * vx;
      y_close = y + c_time * vy;
      if(x_close * x < 0.0 && y < 1.1 * CARLEN) {
         c_time = sdiv((fabs(x) - CARWID), fabs(vx));
         x_close = x + c_time * vx;
         y_close = y + c_time * vy;
      }
    ...
    if(!kount)
      if(lane_inc > .1)
         ...
         lane_inc = 0;
   alpha -= sdiv(abaseadj * lane_inc, width);
