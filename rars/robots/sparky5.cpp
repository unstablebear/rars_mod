...
Robot name:			"Sparky5"
Robot version:		5
Driver function:	Sparky5()
Filename:			Sparky5.cpp
Races:				All
Data file:			None
Robot status:		Public
...
public:
	S5Vec2();
	S5Vec2( double x, double y );
	S5Vec2( const S5Vec2& S5Vec2 );

	S5Vec2	operator+( const S5Vec2& v ) const;
	S5Vec2	operator-( const S5Vec2& v ) const;
	S5Vec2	operator-() const;
	
	S5Vec2	operator*( double n ) const;		// scalar product
	S5Vec2	operator/( double n ) const;		// scalar divide

	double	operator*( const S5Vec2& v ) const;	// dot product
	double	operator%( const S5Vec2& v ) const;	// cross product

	bool	operator==( const S5Vec2& v ) const;
	bool	operator!=( const S5Vec2& v ) const;

	double	GetLength() const;
	double	GetAngle() const;
	S5Vec2	GetUnit() const;
	S5Vec2	GetNormal() const;

	static S5Vec2		FromAngle( double angle );
...
S5Vec2::S5Vec2( double X, double Y )
:	x(X),
	y(Y)
{
}

S5Vec2::S5Vec2( const S5Vec2& v )
:	x(v.x),
	y(v.y)
{
}

S5Vec2	S5Vec2::operator+( const S5Vec2& v ) const
{
	return S5Vec2(x + v.x, y + v.y);
}
...
S5Vec2	S5Vec2::operator-() const
{
	return S5Vec2(-x, -y);
}

S5Vec2	S5Vec2::operator*( double n ) const
{
	return S5Vec2(x * n, y * n);
}
...
double	S5Vec2::operator*( const S5Vec2& v ) const
{
	return x * v.x + y * v.y;
}
...
bool	S5Vec2::operator==( const S5Vec2& v ) const
{
	return x == v.x && y == v.y;
}
...
double	S5Vec2::GetLength() const
{
	return hypot(x, y);
}

double	S5Vec2::GetAngle() const
{
	return atan2(y, x);
}

S5Vec2	S5Vec2::GetUnit() const
{
	if( x == 0 && y == 0 )
		return S5Vec2();

	double	len = hypot(x, y);
	return S5Vec2(x / len, y / len);
}

S5Vec2	S5Vec2::GetNormal() const
{
	return S5Vec2(-y, x);
}

S5Vec2	S5Vec2::FromAngle( double angle )
{
	return S5Vec2(cos(angle), sin(angle));
}
...
public:
	S5Ellipse();
	S5Ellipse( const S5Vec2& centre, double a, double b, double angle );
	S5Ellipse( const S5Ellipse& ellipse );
	...
	bool	LineCrosses(	const S5Vec2& p, const S5Vec2& v,
							double& t1, double& t2 ) const;
	double	CalcLocalTheta( const S5Vec2& point ) const;
	double	CalcTheta( const S5Vec2& point ) const;
	bool	IsPointInside( const S5Vec2& point ) const;
	S5Vec2	CalcLocalPoint( double theta ) const;
	S5Vec2	CalcPoint( double theta ) const;
	double	CalcRadius( double theta ) const;
	double	CalcThetaForRadius( double radius ) const;
	S5Vec2	CalcTangent( double theta ) const;
	bool	CalcTangentsToPoint(	const S5Vec2& point, double& theta1,
									double& theta2 ) const;
	double	CalcDistance( double theta1, double theta2 ) const;

	bool	EllipsesTouch( const S5Ellipse& ellipse ) const;
	bool	FindCrossingPoints( const S5Ellipse& ellipse,
								double& theta1, double& theta2 ) const;

private:
	S5Vec2	m_centre;	// centre of ellipse in global coords
	...
	S5Vec2	m_right;	// local coord system in global coords
	S5Vec2	m_up;		// local coord system in global coords
	S5Vec2	m_f1;		// 1st focal point in global coords
	S5Vec2	m_f2;		// 2nd focal point in global coords
};
...
:	m_centre(centre),
	...
	m_e(m_c / a),
	m_angle(angle),
	m_right(S5Vec2::FromAngle(angle)),
...
S5Ellipse::S5Ellipse( const S5Ellipse& ellipse )
...
S5Vec2	S5Ellipse::Centre() const
{
	return m_centre;
}

double	S5Ellipse::XRadius() const
{
	return m_a;
}

double	S5Ellipse::YRadius() const
{
	return m_b;
}

double	S5Ellipse::Angle() const
{
	return m_angle;
}

double	S5Ellipse::Ratio() const
{
	return m_ratio;
}

bool	S5Ellipse::LineCrosses(
	const S5Vec2& line, const S5Vec2& lineDir, double& t1, double& t2 ) const
{
	// translate line into ellipse coord system
	S5Vec2	p = line - m_centre;
	p = S5Vec2(m_right * p, m_up * p);
	S5Vec2	v(m_right * lineDir, m_up * lineDir);
...
double	S5Ellipse::CalcLocalTheta( const S5Vec2& point ) const
{
	double	theta = atan2(point.y * m_ratio, point.x);
	return theta >= 0 ? theta : theta + cPi * 2;
}

double	S5Ellipse::CalcTheta( const S5Vec2& point ) const
{
	// translate point into local ellipse coord system
	S5Vec2	p = point - m_centre;
	p = S5Vec2(m_right * p, m_up * p);

	return CalcLocalTheta(p);
}

bool	S5Ellipse::IsPointInside( const S5Vec2& point ) const
{
	// for a point to be inside the ellipse the sum of the
	//	distances from both focal points must be <= 2a.
	S5Vec2	f1 = point - m_f1;
	S5Vec2	f2 = point - m_f2;
	double	d1 = sqrt(f1 * f1);
	double	d2 = sqrt(f2 * f2);
	return d1 + d2 <= 2 * m_a;
}

S5Vec2	S5Ellipse::CalcLocalPoint( double theta ) const
{
	return S5Vec2(m_a * cos(theta), m_b * sin(theta));
}

S5Vec2	S5Ellipse::CalcPoint( double theta ) const
{
	S5Vec2	pt = CalcLocalPoint(theta);

	// need to translate point into global coord system
	S5Vec2	aright(m_right.x, m_up.x);
	S5Vec2	aup(m_right.y, m_up.y);
	S5Vec2	apt(aright * pt, aup * pt);
	return m_centre + apt;
}

double	S5Ellipse::CalcRadius( double theta ) const
{
/*
...
double	S5Ellipse::CalcThetaForRadius( double radius ) const
{
...
S5Vec2	S5Ellipse::CalcTangent( double theta ) const
{
	double	sine = sin(theta);
	double	cosine = cos(theta);
	...
	S5Vec2	tangent(-m_a * sine / denom, m_b * cosine / denom);

	// need to translate tangent into global coord system
	S5Vec2	aright(m_right.x, m_up.x);
	S5Vec2	aup(m_right.y, m_up.y);
	S5Vec2	atangent(aright * tangent, aup * tangent);
	return atangent;
}

bool	S5Ellipse::CalcTangentsToPoint(
	const S5Vec2& point, double& theta1, double& theta2 ) const
{
	// translate point into local ellipse coord system
	S5Vec2	p = point - m_centre;
	p = S5Vec2(m_right * p, m_up * p);
	...
	double	deltaTheta ) const
{
	...
	double	delta = cPi / 16;
	S5Vec2	pt1;
	S5Vec2	pt2 = CalcLocalPoint(startTheta);
...
bool	S5Ellipse::EllipsesTouch( 
	const S5Ellipse&	ellipse ) const
{
...
//	double fb = f1b + f2b;
	...
	S5Vec2	d = ellipse.m_centre - m_centre;
	double	d2 = d * d;
	...
	for( double delta = 0; !done; delta += step )
	{
		...
		S5Vec2	pt = CalcPoint(startTheta + delta);
...
bool	S5Ellipse::FindCrossingPoints( 
	const S5Ellipse&	ellipse,
	double&			retTheta1,
	double&			retTheta2 ) const
{
	...
	int		nFound = 0;
	S5Vec2	pt2 = CalcPoint(startTheta);
...
static double	CalcK( double r, double b, double /*apexLift*/ )
{
	double	K = cEllRatio;
	if( b < r * K )
			K = max(1, b / r);
...
static double	CalcClosestPointOnLine( const S5Vec2& lp, const S5Vec2& lv, const S5Vec2& p )
{
	double	denom = lv * lv;
	if( denom == 0 )
		return 0;
...
static double	CalcDistanceToLine( const S5Vec2& lp, const S5Vec2& lv, const S5Vec2& p )
{
...
/* Not used
static S5Vec2	LineCrossesLinePt(
	const S5Vec2&	lp0,
	const S5Vec2& lv0,
	const S5Vec2&	lp1,
	const S5Vec2& lv1 )
...
static bool	LineCrossesCircle(
	const S5Vec2& lp,		// start point of line
	const S5Vec2& lv,		// vector for line
	const S5Vec2& cp,		// centre point of circle
...
/* Not used
static bool	CircleCrossesCircle(
	const S5Vec2&	c1,
	double		r1,
	const S5Vec2&	c2,
	double		r2,
...
static bool	CalcTangentPt(
	const S5Vec2& pt,		// point to calc tangent to
	const S5Vec2& cen,	// centre of circle
	double		rad,	// radius, -ve if clockwise
	S5Vec2&		tanPt )	// point calculated
{
	S5Vec2	v = cen - pt;
	double	r = fabs(rad);
	...
	S5Vec2	v2 = S5Vec2::FromAngle(beta);
...
static bool	CalcTangentPt(
	const S5Vec2& pt,		// point to calc tangent to
	const S5Ellipse& ell,	// ellipse for tangent
	...
	double& tanTheta )	// theta of point
{
	...
	S5Vec2	pt1 = ell.CalcPoint(theta1);
	S5Vec2	pt2 = ell.CalcPoint(theta2);

	S5Vec2	n = (ell.Centre() - pt).GetNormal();
...
static bool	LineCrossesSegment(
	const S5Vec2&		p,
	const S5Vec2&		v,
	const segment*	pSeg,
	double			edge,
	double&			t )
{
	S5Vec2	b(pSeg->beg_x, pSeg->beg_y);

	if( pSeg->radius == 0 )
	{
		S5Vec2	e(pSeg->end_x, pSeg->end_y);
	...
	{
		S5Vec2	c(pSeg->cen_x, pSeg->cen_y);
...
/* Not used
static void	CalcCurve(
	double rad, double len, double width, double apex, double out,
	double& newRad, S5Vec2& newCen )
{
	double	r = fabs(rad);
	S5Vec2	c(-r, 0);
	double	nr = r + apex;
	S5Vec2	p(cos(len * 0.5) * nr - r, sin(len * 0.5) * nr);
	S5Vec2	pt = (c - p).GetNormal();
	S5Vec2	q = LineCrossesLinePt(p, pt, S5Vec2(width - out, 0), S5Vec2(0, 1));
	double	h = (p - q).GetLength();
	S5Vec2	s(q.x, q.y - h);
	S5Vec2	c2 = LineCrossesLinePt(s, S5Vec2(-1, 0), c, c - p);
	double	r2 = s.x - c2.x;
...
static void	CalcEllipse(
	...
	S5Ellipse&		ellipse,	// ellipse calculated
...
again:
	...
	S5Vec2	f(c + S5Vec2::FromAngle(fromApex) * (r + width - out));
	S5Vec2	f1(f.x, f.y * ellRatio);

	// tangent at a
	S5Vec2	ft = (f - c).GetNormal();
	S5Vec2	f1t = S5Vec2(ft.x, ft.y * ellRatio).GetUnit();

	// calculate tangent at apex
	S5Vec2	pt(0, 1);

	// figure where tangent crosses outer line
	S5Vec2	q1 = LineCrossesLinePt(p, pt, f1, f1t);
	...
	//	outer line
	S5Vec2	s1 = q1 + f1t * h;
	S5Vec2	s(s1.x, s1.y / ellRatio);

	// work out normal at this point
	S5Vec2	s1n = f1t.GetNormal();

	// work out the centre of the new circle
	S5Vec2	c2 = LineCrossesLinePt(s1, s1n, c, c - p);
	double	r2 = p.x - c2.x;

	// work out global coord vector along the major axis
	double	globalApexAngle = (S5Vec2(pSeg->beg_x, pSeg->beg_y) -
								S5Vec2(pSeg->cen_x, pSeg->cen_y)).GetAngle();
	if( pSeg->radius > 0 )
		globalApexAngle += apexAngle;
	else
		globalApexAngle -= apexAngle;
	S5Vec2	v = S5Vec2::FromAngle(globalApexAngle);

	// work out ellipse in local coords
	double	a = r2;
	double	b = r2 / ellRatio;
	S5Ellipse	localEll(c2, a, b, 0);
	...
	//	global coordinates
	S5Vec2	cen = S5Vec2(pSeg->cen_x, pSeg->cen_y) - v * (a - r - apex);

	// fill in the ellipse and the value of theta for the
	//	outer tangent point
	ellipse = S5Ellipse(cen, a, b, globalApexAngle);
...
static bool	EllipseCrossesSegment(
	const S5Ellipse& ell,		// ellipse to test
	...
	double&	theta )			// theta value of crossing point
{
	...
	if( seg.radius == 0 )
	{
		...
		S5Vec2	b(seg.beg_x, seg.beg_y);
		S5Vec2	e(seg.end_x, seg.end_y);
		S5Vec2	v = (e - b).GetUnit();
		S5Vec2	n = v.GetNormal();
	...
	else
	{
		...
		S5Vec2	p2 = c + S5Vec2::FromAngle(startAngle) * r;
		double	step = cPi / 32;
		bool	done = false;
...
static bool	IsSegBefore( int seg1, int seg2, int nSeg )
{
	if( seg1 < seg2 )
	{
...
public:
	...
	S5Vec2	m_apexDir;	// centre to apex direction vector
	S5Vec2	m_apexPt;	// point for apex
...
public:
	...
	S5Vec2			LftBeg( int seg ) const;
	S5Vec2			LftEnd( int seg ) const;
	S5Vec2			LftCen( int seg ) const;
	S5Vec2			RgtBeg( int seg ) const;
	S5Vec2			RgtEnd( int seg ) const;
	S5Vec2			RgtCen( int seg ) const;

	const S5Info&		GetInfo( int seg ) const;
	S5Info&			GetInfo( int seg );

	double			GetRadius( int seg ) const;
	double			GetLength( int seg ) const;

	bool			IsAngleInSegment( int seg, double angle ) const;
	bool			SegContainsPt( int seg, const S5Vec2& pt ) const;
	int				FindSegContainingPt(	const S5Vec2& pt, int startSeg = 0,
...
private:
	...
	bool			EllipseCrosses( const S5Ellipse& ell, int ellSeg,
...
void	S5Track::Initialise( const track_desc& track )
{
	Empty();

	m_track = track;
	m_pInfo = new S5Info[m_track.NSEG];
	m_nApexes = 0;
...
int		S5Track::NSeg() const
{
	return m_track.NSEG;
}

double	S5Track::Width() const
{
	return m_track.width;
}

const track_desc&	S5Track::GetTrack() const
{
	return m_track;
}

track_desc&	S5Track::GetTrack()
{
	return m_track;
}
...
const segment&	S5Track::LftWall( int seg ) const
{
...
segment&	S5Track::LftWall( int seg )
{
	return m_track.lftwall[seg];
}
...
const segment&	S5Track::RgtWall( int seg ) const
{
...
segment&	S5Track::RgtWall( int seg )
{
	return m_track.rgtwall[seg];
}
...
const S5Info&	S5Track::GetInfo( int seg ) const
{
	return m_pInfo[seg];
}
...
S5Vec2	S5Track::LftBeg( int seg ) const
{
	return S5Vec2(m_track.lftwall[seg].beg_x, m_track.lftwall[seg].beg_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Vec2	S5Track::LftEnd( int seg ) const
{
	return S5Vec2(m_track.lftwall[seg].end_x, m_track.lftwall[seg].end_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Vec2	S5Track::LftCen( int seg ) const
{
	return S5Vec2(m_track.lftwall[seg].cen_x, m_track.lftwall[seg].cen_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Vec2	S5Track::RgtBeg( int seg ) const
{
	return S5Vec2(m_track.rgtwall[seg].beg_x, m_track.rgtwall[seg].beg_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Vec2	S5Track::RgtEnd( int seg ) const
{
	return S5Vec2(m_track.rgtwall[seg].end_x, m_track.rgtwall[seg].end_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Vec2	S5Track::RgtCen( int seg ) const
{
	return S5Vec2(m_track.rgtwall[seg].cen_x, m_track.rgtwall[seg].cen_y);
}

/////////////////////////////////////////////////////////////////////////////

S5Info&	S5Track::GetInfo( int seg )
{
	return m_pInfo[seg];
}
...
double	S5Track::GetRadius( int seg ) const
{
...
double	S5Track::GetLength( int seg ) const
{
...
bool	S5Track::IsAngleInSegment( int seg, double angle ) const
{
...
bool	S5Track::SegContainsPt( int seg, const S5Vec2& pt ) const
{
	if( m_track.lftwall[seg].radius == 0 )
	{
		...
		// work out mid line
		S5Vec2	b((LftBeg(seg) + RgtBeg(seg)) * 0.5);
		S5Vec2	e((LftEnd(seg) + RgtEnd(seg)) * 0.5);
...
int		S5Track::FindSegContainingPt(
	const S5Vec2& pt,
	int startSeg,
	bool forward ) const
{
...
void	S5Track::CopyTrack( const track_desc& track )
{
	m_track = track;
	...
	for( int seg = 0; seg < m_track.NSEG; seg++ )
	{
		...
		{
			// on a straight
			S5Vec2	b(track.lftwall[seg].beg_x, track.lftwall[seg].beg_y);
			S5Vec2	e(track.lftwall[seg].end_x, track.lftwall[seg].end_y);
			S5Vec2	v((e - b).GetUnit());
			S5Vec2	n(v.GetNormal() * cTrackBuffer);	// points to left
		...
		{
			S5Vec2	c(track.lftwall[seg].cen_x, track.lftwall[seg].cen_y);
			S5Vec2	b(track.lftwall[seg].beg_x, track.lftwall[seg].beg_y);
			S5Vec2	e(track.lftwall[seg].end_x, track.lftwall[seg].end_y);

			S5Vec2	begN = (b - c).GetUnit();
			S5Vec2	endN = (e - c).GetUnit();
	...
	S5Vec2	b(track.lftwall[0].beg_x, track.lftwall[0].beg_y);
	S5Vec2	e(track.lftwall[0].end_x, track.lftwall[0].end_y);
...
void	S5Track::CalcApex( int seg, double inBuffer, double outBuffer )
{
	if( m_track.lftwall[seg].radius == 0 )
		return;
...
//	double	minR = fabs(pInner->radius) + apexLift;
	...
	S5Vec2	c = S5Vec2(pInner->cen_x, pInner->cen_y);
	S5Vec2	b = S5Vec2(pInner->beg_x, pInner->beg_y);
	// S5Vec2	e = S5Vec2(pInner->end_x, pInner->end_y);
...
//		hiInitAngle = hiInitAngle * 0.5;
	...
	while( loB < hiB )
	{
		...
		S5Vec2	v = S5Vec2::FromAngle(midAngle);
		S5Vec2	cen = c - v * (midA - (radius + apexLift));

		S5Ellipse	ell(cen, midA, midB, midAngle);
...
void	S5Track::AdjustSBends( int apexesBetween )
{
	...
	while( !done )
	{
		...
		for( int i = 0; i < m_nApexes; i++ )
		{
			...
			S5Ellipse&	seg1Ell = m_pInfo[seg1].m_ell;
			S5Ellipse&	seg2Ell = m_pInfo[seg2].m_ell;
...
struct	S5Situation
{
	S5Vec2	m_carPt;	// global car coordinates
	S5Vec2	m_carDir;	// global car unit direction vector
	double	m_carAngle;	// global car angle
	double	m_tanDist;	// distance till tangent point

	S5Vec2	m_distDir;	// direction to most distant point down track
	bool	m_nextTurnIsLeft;	// true if will be turning left
};

/////////////////////////////////////////////////////////////////////////////

static bool	CalcCrossDistance(
	const S5Vec2&	p,
	const S5Vec2& v,
	int startSeg,
	...
	S5Vec2& aheadTanPt )
{
	...
	aheadDist = 0;
	aheadDir = S5Vec2::FromAngle(carAngle);
	...
	for( int i = 0; i < max; i++ )
	{
		...
		if( !CalcTangentPt(	car, S5Vec2(pSeg->cen_x, pSeg->cen_y), rad, tanPt) )
			continue;

		// find direction vector to tangent point
		S5Vec2	v = (tanPt - car).GetUnit();
...
//	newTanDist = 0;
	...
	const S5Info*	pApexInfo = &track.GetInfo(apexSeg);
...
/*	if( pApexInfo->m_ell.IsPointInside(carPt) )
	{
		...
		while( minB + 0.1 < maxB )
		{
			...
			double	a = b * K;
			S5Vec2	c = pApexInfo->m_apexPt - pApexInfo->m_apexDir * a;
			S5Ellipse	ell(c, a, b, pApexInfo->m_ell.Angle());
...
*/
	for( double	b = maxB; b >= minR; b *= 0.95 )
	{
		...
		const S5Info&	info = track.GetInfo(apexSeg);
...
void	AvoidOtherCarsOld(
	...
	const S5Situation&	mySituation,	// global situation values
	const S5Track&		track,			// track
...
static bool	CheckOtherIn( const S5Vec2& p, double sine, double cosine )
{
	S5Vec2	l = S5Vec2(cosine, sine) * (CARLEN / 2);
	S5Vec2	w = S5Vec2(sine, -cosine) * (CARWID / 2);
...
static bool	CheckCollide( const S5Vec2& p, double angle )
{
	...
	S5Vec2	p2(p.y * c - p.x * s, -p.x * c - p.y * s);
...
//				needToBrake = true;
		...
		for( double time = 0; time < 3; time += deltaT )
		{
			...
			S5Vec2	p1 = cen + S5Vec2::FromAngle(a1) * rad;

			double	a2 = relAngle + al * time;
			S5Vec2	p2 = c + S5Vec2::FromAngle(a2) * r;
			...
			if( dist < minDist )
			{
				...
				minTime = time;
				minP1 = p1;
				minDir = S5Vec2::FromAngle(a1 + cPi_2);
				minP2 = p2;
	...
	if( collideTime < 10 )
	{
		S5Vec2	norm = collideDir1.GetNormal();
...
//		urgency = min(urgency, 1);
		double	urgency = min(1, sqrt(max(1 - collideTime / 3, 0)));
...
con_vec Sparky5( situation& s )
{ 
	const char name[] = "Sparky5";	// This is the robot driver's name! 

	static S5Track		track;		// track details
	...
	S5Vec2	tangent;	// track direction we desire to be moving in
...
//	SparkyToLeft = s.to_lft;
	...
	else
	{
		...
		S5Vec2	v(S5Vec2::FromAngle(carAng));
	...
	S5Vec2	carDir = S5Vec2::FromAngle(carAngle);

	// look ahead to see how far we can see in a straight line
	double	distAhead = 0;
	S5Vec2	distDir(carDir);
	int		distSeg;
	int		tanSeg = -1;
	S5Vec2	tanPt;
	double	tanDist = 0;
...
*/
		...
		S5Ellipse apexEll;
		// double	delta = 0;
		S5Vec2	newTanPt(tanPt);
		S5Vec2	newTanDir(distDir);
		double	newTanDist = 0;
...
//			if( newTanDist > dist * 5 && newTanDist > 700 )
			if( newTanDist > dist * 2 )
			{
										...
										exitEllPt, exitEllDir, exitEllDist) )
				{
					...
					S5Vec2	carNorm = carDir.GetNormal();
		...
		if( betterLine )
		{
			...
			S5Vec2	p = apexEll.CalcPoint(carTheta);
...
/*			if( distFromEll < 1 )
			{
				...
				S5Vec2	ellTan = apexEll.CalcTangent(carTheta);
				if( !apexIsLeft )
					ellTan = -ellTan;
				double	tanAngle = ellTan.GetAngle();
				S5Vec2	tanNorm = ellTan.GetNormal();
				...
				distDir = S5Vec2::FromAngle(carAngle);
...
*/
	...
	//		pInfo[apexSeg].m_ell = apexEll;
				...
				apexEll.Angle() == track.GetInfo(apexSeg).m_ell.Angle() )
			{
				S5Vec2	tanPt;
				double	tanTheta;
				S5Info&	info = track.GetInfo(apexSeg);
...
*/
	...
	S5Vec2	slipVec = S5Vec2(s.v, 0) - S5Vec2::FromAngle(alpha) * s.vc;
...
//			vc = s.v * 0.8;
		...
		vc = s.v * bc;
		S5Vec2	alphaVec = S5Vec2::FromAngle(alpha);
		S5Vec2	slipVec = S5Vec2(s.v, 0) - alphaVec * vc;
		double	slip = slipVec.GetLength();
		double	myu = friction(slip);
		...
		while( bc < 0.95 && myuN < minMyuN )
		{
			bc += 0.01;
			vc = s.v * bc;
			slipVec = S5Vec2(s.v, 0) - alphaVec * vc;
